...
void mag_assign_spells(void) {
  ...
  spello(SPELL_BLINDNESS, "blindness", 35, 25, 1, 10, POS_STANDING,
  ...
  spello(SPELL_GROUP_ARMOR, "group armor", 50, 30, 2, 1, POS_STANDING,
  ...
  spello(SPELL_GROUP_HEAL, "group heal", 80, 60, 5, 1, POS_STANDING,
  TAR_IGNORE, FALSE, MAG_GROUPS, NULL);
...
spello(SPELL_STAMINA_PERCENT, "stamina percent", 60, 40, 3, 1, POS_FIGHTING,
  ...
  spello(SPELL_INVISIBLE, "invisibility", 35, 25, 1, 1, POS_STANDING,
  ...
  skillo(SKILL_BACKSTAB, 120, "backstab");
  skillo(SKILL_BASH, 120, "bash");
  skillo(SKILL_HIDE, 1, "hide");
  skillo(SKILL_KICK, 3, "kick");
  skillo(SKILL_PICK_LOCK, 10, "pick lock");
  skillo(SKILL_RESCUE, 1, "rescue");
  skillo(SKILL_SNEAK, 1, "sneak");
  skillo(SKILL_STEAL, 120, "steal");
  skillo(SKILL_TRACK, 10, "track");
  skillo(SKILL_WHIRLWIND, 10, "whirlwind");
  skillo(SKILL_BANDAGE, 10, "bandage");
}
