...
/*void display_account_menu(struct descriptor_data *d) {
  ...
  if (d->account && d->account->num_characters > 0) {
    for (int i = 0; i < d->account->num_characters; i++) {
      write_to_output(d, "  %d) %s\r\n", i + 1, d->account->characters[i]);
    }
  } else {
    write_to_output(d, "  (No characters yet)\r\n");
  }

  write_to_output(d, "  N) Create New Character\r\n");
...
void display_account_menu(struct descriptor_data *d) {
  write_to_output(d, "\r\nChoose a character:\r\n");
  write_to_output(d,
    "Num Name         Lvl  Class      Last Login\r\n"
    "--- ------------ ---- ---------- ------------------------\r\n");

  for (int i = 0; i < d->account->num_characters; i++) {
    struct char_data *temp;
    char loginstr[20] = "N/A";
    const char *name = d->account->characters[i];

    CREATE(temp, struct char_data, 1);
    clear_char(temp);
...
void three_arguments(char *argument, char *first, char *second, char *third) {
  char *temp = any_one_arg(argument, first);
  temp = any_one_arg(temp, second);
  any_one_arg(temp, third);
}
...
cpp_extern const struct command_info cmd_info[] = {
  ...
  { "damagenumbers", "damnr", POS_DEAD, do_damagenumbers, 0, 0 },
  ...
  { "score1"    , "score1"      , POS_DEAD    , do_score1    , 0, 0 },
  { "scorebin"    , "sbin"      , POS_DEAD    , do_score_bin    , 0, 0 },
  ...
  { "setaccount","setacc"  , POS_DEAD    , do_setaccount, LVL_IMPL, 0 },
  ...
  { "stash"    , "stash"   , POS_RESTING , do_stash    , 1, 0 },
...
void nanny(struct descriptor_data *d, char *arg)
{
  ...
  case CON_NAME_CNFRM:		/* wait for conf. of new name    */
	...
	return;
      ...
      //strncpy(GET_PASSWD(d->character), "", MAX_PWD_LENGTH);  // optional: disable password check logic
  ...
  case CON_CHPWD_VRFY:
    ...
    if (STATE(d) == CON_CNFPASSWD) {
      ...
      if (d->account && d->character) {
        if (account_add_character(d->account, GET_NAME(d->character))) {
  ...
  case CON_QCLASS:
    ...
    /* Link new character to account */
    if (d->account && d->character) {
      const char *name = GET_NAME(d->character);
      if (!account_has_character(d->account, name)) {
        if (account_add_character(d->account, name)) {
          save_account(d->account);
        }
      }
    }
    ...
    case CON_MENU: {
      ...
      case '4':
        write_to_output(d, "\r\nPassword changes must be done from the account menu.\r\n");
        display_account_menu(d);
        STATE(d) = CON_ACCOUNT_CHAR_SELECT;
        break;
  
      case '5':
        write_to_output(d, "\r\nCharacter deletion must be performed through the account menu.\r\n");
        display_account_menu(d);
        STATE(d) = CON_ACCOUNT_CHAR_SELECT;
        break;
  
      case '6':
        write_to_output(d, "Returning to account menu...\r\n");
        display_account_menu(d);
        STATE(d) = CON_ACCOUNT_CHAR_SELECT;
        break;
  ...
  case CON_CLOSE:
    ...
    if (!*arg) {
      write_to_output(d, "Account name cannot be empty. Try again:\r\nAccount Name: ");
      return;
    }
    for (const char *p = arg; *p; p++) {
      if (!isalnum(*p)) {
        write_to_output(d, "Account names can only contain letters and numbers. Try again:\r\nAccount Name: ");
        return;
      }
    }

    if (strchr(arg, ' ')) {
      write_to_output(d, "Account names cannot contain spaces. Try again:\r\nAccount Name: ");
      return;
    }

    strncpy(d->account_name, arg, sizeof(d->account_name) - 1);
    d->account_name[sizeof(d->account_name) - 1] = '\0';

    if ((d->account = load_account(d->account_name))) {
      if (!d->account->siteok && !CONFIG_SITEOK_ALL) {
        mudlog(BRF, LVL_IMMORT, TRUE, "Login denied from %s to account %s (siteok off).",
               d->host, d->account->name);
        write_to_output(d, "This account is not allowed to log in from this site.\r\n");
        STATE(d) = CON_CLOSE;
        return;
      }
      write_to_output(d, "Password: ");
      ...
      write_to_output(d, "No such account. Create new account? (y/n): ");
      STATE(d) = CON_ACCOUNT_CHECK_EXIST;
    }
    break;

  

case CON_ACCOUNT_CHECK_EXIST:
  if (LOWER(*arg) == 'y') {
    write_to_output(d, "Enter new password: ");
    echo_off(d);
    STATE(d) = CON_ACCOUNT_NEW_PASS;
  } else {
...
case CON_ACCOUNT_NEW_PASS:
  if (!*arg) {
    write_to_output(d, "Enter new password: ");
    return;
  }
  strncpy(d->account_password, arg, sizeof(d->account_password) - 1);
  write_to_output(d, "Confirm password: ");
  STATE(d) = CON_ACCOUNT_CONFIRM_PASS;
  break;

  case CON_ACCOUNT_CONFIRM_PASS:
  if (strcmp(arg, d->account_password)) {
    write_to_output(d, "Passwords don't match. Enter new password: ");
    STATE(d) = CON_ACCOUNT_NEW_PASS;
    return;
  }
  d->account = create_account(d->account_name, d->account_password);
  if (!d->account || !save_account(d->account)) {
    write_to_output(d, "Account creation failed. Try again.\r\nAccount Name: ");
    STATE(d) = CON_ACCOUNT_NAME;
    return;
  }
  write_to_output(d, "Account created successfully!\r\n");
  display_account_menu(d);
  STATE(d) = CON_ACCOUNT_CHAR_SELECT;
  break;
...
case CON_ACCOUNT_PASSWORD:
  echo_on(d);
  write_to_output(d, "\r\n");  // in case echo_on ate the newline

  if (!d->account || !verify_password(d->account, arg)) {
    d->bad_account_pws++;

    if (d->bad_account_pws >= CONFIG_MAX_BAD_PWS) {
      mudlog(BRF, LVL_IMMORT, TRUE, "Account login failed too many times from %s", d->host);
      write_to_output(d, "Too many failed attempts. Disconnecting.\r\n");
      STATE(d) = CON_CLOSE;
      return;
    }
  ...
  if (!d->account->siteok && !CONFIG_SITEOK_ALL) {
    mudlog(BRF, LVL_IMMORT, TRUE, "Connection from %s denied to account %s (site not ok).", 
           d->host, d->account->name);
    write_to_output(d, "Sorry, your account is not allowed to log in from this site.\r\n");
    STATE(d) = CON_CLOSE;
    return;
  }

  // Successful login
  d->bad_account_pws = 0;   // Reset failed attempts counter
  update_last_login(d->account);
  save_account(d->account);

  write_to_output(d, "\r\nLogin successful!\r\n");
  mudlog(NRM, LVL_IMMORT, TRUE, "Account %s logged in from %s", d->account->name, d->host);
...
write_to_output(d, "\r\nChoose a character:\r\n");
  for (int i = 0; i < d->account->num_characters; i++) {
    ...
    write_to_output(d, "%s", line);
  }
  write_to_output(d, "  N) Create New Character\r\nEnter choice: ");
  STATE(d) = CON_ACCOUNT_CHAR_SELECT;
  break;

  case CON_ACCOUNT_CHAR_SELECT: {
    if (LOWER(*arg) == 'n') {
      write_to_output(d, "Enter your new character's name: ");
      STATE(d) = CON_GET_NAME;
      return;
    }
    if (LOWER(*arg) == 'd') {
      if (d->account->num_characters == 0) {
        write_to_output(d, "You have no characters to delete.\r\n");
        display_account_menu(d);
        return;
      }
      write_to_output(d, "Enter the number of the character you wish to delete: ");
      STATE(d) = CON_ACCOUNT_CHAR_DELETE_SELECT;
      return;
    }
    ...
    if (LOWER(*arg) == 'p') {
      ...
      echo_off(d);
      STATE(d) = CON_ACCOUNT_CHANGEPASS_OLD;
      return;
    }

    if (LOWER(*arg) == 'q') {
      write_to_output(d, "Goodbye.\r\n");
      mudlog(NRM, LVL_IMMORT, TRUE, "Account %s disconnected from account menu.", d->account->name);
      STATE(d) = CON_CLOSE;
      return;
    }

    int choice = atoi(arg) - 1;
    if (choice < 0 || choice >= d->account->num_characters) {
      write_to_output(d, "Invalid choice. Try again: ");
      return;
    }
  
    const char *name = d->account->characters[choice];
    mudlog(NRM, LVL_IMMORT, TRUE, "DEBUG: Account %s selected character %d: %s", 
           d->account->name, choice, name);
  
    // Check max online characters for this account
    int active = 0;
    for (struct descriptor_data *dtemp = descriptor_list; dtemp; dtemp = dtemp->next) {
      if (dtemp != d && dtemp->account &&
       d->account && strcmp(dtemp->account->name, d->account->name) == 0 &&
        STATE(dtemp) == CON_PLAYING) {
      active++;
      }
    }

if (active >= d->account->max_online) {
  write_to_output(d, "Too many characters from this account are already logged in (%d allowed).\r\n",
                  d->account->max_online);
  display_account_menu(d);
  return;
}


    // Allocate and initialize character
    CREATE(d->character, struct char_data, 1);
    clear_char(d->character);
    CREATE(d->character->player_specials, struct player_special_data, 1);
    new_mobile_data(d->character);
    GET_HOST(d->character) = strdup(d->host);
    d->character->desc = d;
    d->character->account = d->account;
    write_to_output(d, "\r\nLogin successful! (Account: %s)\r\n", d->account->name);
  
    // Load character data
    int load_result = load_char(name, d->character);
    d->character->account = d->account;
    if (load_result < 0) {
      write_to_output(d, "Error loading character '%s'. Returning to menu.\r\n", name);
      free(d->character);
      d->character = NULL;
      display_account_menu(d);
      STATE(d) = CON_ACCOUNT_CHAR_SELECT;
    ...
    if (!AddRecentPlayer(GET_NAME(d->character), d->host, FALSE, FALSE)) {
      mudlog(BRF, LVL_IMMORT, TRUE, "Failure to AddRecentPlayer for %s.", GET_NAME(d->character));
    }
  ...
  case CON_ACCOUNT_CHAR_DELETE_SELECT: {
    int choice = atoi(arg) - 1;
    ...
    d->character_to_delete = choice;
    write_to_output(d, "Are you sure you want to delete %s? Type YES to confirm: ",
      d->account->characters[choice]);
    STATE(d) = CON_ACCOUNT_CHAR_DELETE_CONFIRM;
    break;
  }
    

  case CON_ACCOUNT_CHAR_DELETE_CONFIRM: {
    if (str_cmp(arg, "yes") != 0) {
    ...
    int idx = d->character_to_delete;
    const char *name = d->account->characters[idx];
    ...
    if (load_char(name, victim) >= 0) {
      if (GET_PFILEPOS(victim) < 0)
        GET_PFILEPOS(victim) = create_entry(GET_NAME(victim));
  
      if (PLR_FLAGGED(victim, PLR_FROZEN)) {
        write_to_output(d, "This character is frozen and cannot be deleted.\r\n");
        free_char(victim);
        display_account_menu(d);
        STATE(d) = CON_ACCOUNT_CHAR_SELECT;
        break;
      ...
      save_char(victim);
      Crash_delete_file(GET_NAME(victim));  // Delete .objs file
  
  #ifdef selfdelete_fastwipe
      if (selfdelete_fastwipe) {
        int player_i = get_ptable_by_name(GET_NAME(victim));
        if (player_i >= 0) {
          SET_BIT(player_table[player_i].flags, PINDEX_SELFDELETE);
          remove_player(player_i);
        }
      ...
      delete_variables(GET_NAME(victim));
  
      write_to_output(d, "Character '%s' marked for deletion.\r\n", name);
      mudlog(NRM, LVL_IMMORT, TRUE, "Account %s flagged character '%s' for deletion.",
             d->account->name, name);
    } else {
      write_to_output(d, "Could not load character '%s'. File missing?\r\n", name);
    }
  
    free_char(victim);
  
    // Remove from account
    free(d->account->characters[idx]);
    for (int i = idx; i < d->account->num_characters - 1; i++)
      d->account->characters[i] = d->account->characters[i + 1];
    d->account->characters[--d->account->num_characters] = NULL;
  ...
  case CON_ACCOUNT_CHANGEPASS_OLD:
  if (strncmp(CRYPT(arg, d->account->password), d->account->password, MAX_PWD_LENGTH)) {
    ...
    write_to_output(d, "\r\nEnter a new password: ");
    STATE(d) = CON_ACCOUNT_CHANGEPASS_NEW;
  }
  return;

case CON_ACCOUNT_CHANGEPASS_NEW:
  if (!*arg || strlen(arg) > MAX_PWD_LENGTH || strlen(arg) < 3 ||
  ...
  strncpy(d->account_password, arg, sizeof(d->account_password) - 1);
  d->account_password[sizeof(d->account_password) - 1] = '\0';

  write_to_output(d, "\r\nPlease retype new password: ");
  STATE(d) = CON_ACCOUNT_CHANGEPASS_CONFIRM;
  return;

case CON_ACCOUNT_CHANGEPASS_CONFIRM:
  if (strncmp(CRYPT(arg, d->account_password), CRYPT(d->account_password, d->account->name), MAX_PWD_LENGTH)) {
    write_to_output(d, "\r\nPasswords don't match... start over.\r\nNew password: ");
    STATE(d) = CON_ACCOUNT_CHANGEPASS_NEW;
    return;
  }
  strncpy(d->account->password, CRYPT(d->account_password, d->account->name), ACCOUNT_PASS_LENGTH - 1);
  d->account->password[ACCOUNT_PASS_LENGTH - 1] = '\0';
  echo_on(d);

  save_account(d->account);
  write_to_output(d, "\r\nPassword changed successfully.\r\n");
  mudlog(NRM, LVL_IMMORT, TRUE, "Account %s changed their password.", d->account->name);
  display_account_menu(d);
  STATE(d) = CON_ACCOUNT_CHAR_SELECT;
  break;
