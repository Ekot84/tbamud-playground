...
ACMD(do_setaccount) {
  char accountname[MAX_INPUT_LENGTH], field[MAX_INPUT_LENGTH], value[MAX_INPUT_LENGTH];
  struct account_data *account;

  three_arguments(argument, accountname, field, value);

  if (!*accountname || !*field || !*value) {
    send_to_char(ch, "Usage: setaccount <accountname> <field> <value>\r\n");
    send_to_char(ch, "Fields: password, email, siteok\r\n");
    return;
  }

  account = load_account(accountname);
  if (!account) {
    send_to_char(ch, "Account '%s' not found.\r\n", accountname);
    return;
  }

  if (!strcasecmp(field, "password")) {
    strncpy(account->password, CRYPT(value, account->name), ACCOUNT_PASS_LENGTH - 1);
    account->password[ACCOUNT_PASS_LENGTH - 1] = '\0';
    send_to_char(ch, "Password for account '%s' updated.\r\n", account->name);
  }
  else if (!strcasecmp(field, "email")) {
    strncpy(account->email, value, ACCOUNT_EMAIL_LENGTH - 1);
    account->email[ACCOUNT_EMAIL_LENGTH - 1] = '\0';
    send_to_char(ch, "Email for account '%s' set to '%s'.\r\n", account->name, account->email);
  }
  else if (!strcasecmp(field, "siteok")) {
    if (!strcasecmp(value, "on"))
      account->siteok = true;
    else if (!strcasecmp(value, "off"))
      account->siteok = false;
    else {
      send_to_char(ch, "Value for siteok must be 'on' or 'off'.\r\n");
      free_account(account);
      return;
    }
    send_to_char(ch, "Siteok for account '%s' is now %s.\r\n", account->name, account->siteok ? "ON" : "OFF");
    } else if (!strcmp(field, "maxonline")) {
    int value_int = atoi(value);
    if (value_int < 0 || value_int > 20) {
      send_to_char(ch, "MaxOnline must be between 0 and 20.\r\n");
    } else {
      account->max_online = value_int;
      send_to_char(ch, "MaxOnline for account '%s' set to %d.\r\n", account->name, account->max_online);
    }
  } else {
    send_to_char(ch, "Unknown field '%s'. Valid fields: password, email, siteok, maxonline\r\n", field);
    free_account(account);
    return;
  }
